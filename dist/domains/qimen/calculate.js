...
exports.calculateQimenData = calculateQimenData;
const lunar_javascript_1 = require("lunar-javascript");
const ganzhi_js_1 = require("../../data/ganzhi.js");
const timezone_utils_js_1 = require("../../shared/timezone-utils.js");
const utils_js_1 = require("../../shared/utils.js");
let taobiConstructorPromise = null;
// 互斥锁：保护 process.env.TZ 全局变异区间，防止并发竞争
let tzMutexQueue = Promise.resolve();
...
function buildMonthPhaseMap(season) {
    const result = {};
    for (const stem of ganzhi_js_1.TIAN_GAN) {
        const element = ganzhi_js_1.GAN_WUXING[stem] || '';
...
function getXunShou(dayStem, dayBranch) {
    const ganIdx = ganzhi_js_1.TIAN_GAN.indexOf(dayStem);
    const zhiIdx = ganzhi_js_1.DI_ZHI.indexOf(dayBranch);
    if (ganIdx < 0 || zhiIdx < 0)
        return '甲子';
    const startZhiIdx = (zhiIdx - ganIdx + 12) % 12;
    return `甲${ganzhi_js_1.DI_ZHI[startZhiIdx]}`;
}
function assertValidTimeZone(timeZone) {
    try {
        (0, timezone_utils_js_1.getTimeZoneOffsetMinutes)(timeZone, new Date());
    }
    catch (error) {
    ...
    return loadTaobiConstructor().then((TheArtOfBecomingInvisible) => {
        ...
        return withTzMutex(() => {
            ...
            try {
                ...
                // 空亡计算
                const dayKong = (0, utils_js_1.getKongWang)(dayGan, dayZhi);
                const hourKong = (0, utils_js_1.getKongWang)(hourGan, hourZhi);
                ...
                // 驿马
                const yiMaBranch = ganzhi_js_1.YI_MA_MAP[dayZhi] || '';
                ...
                for (let i = 0; i < 9; i++) {
                    ...
                    // 旺衰
                    const stemElement = ganzhi_js_1.GAN_WUXING[heavenStem] || '';
                    ...
                    palaces.push({
                        ...
                        earthStem,
                        earthStemElement: ganzhi_js_1.GAN_WUXING[earthStem] || '',
                        heavenStem,
                        heavenStemElement: ganzhi_js_1.GAN_WUXING[heavenStem] || '',
                        star: starName,
