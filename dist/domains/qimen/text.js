...
exports.renderQimenCanonicalText = renderQimenCanonicalText;
const render_utils_js_1 = require("../../shared/render-utils.js");
...
function formatQimenMonthPhaseLine(monthPhase) {
    ...
    for (const [phase, stems] of phaseGroups.entries()) {
        const stemsWithElement = stems.map((stem) => `${stem}${utils_js_1.GAN_WUXING[stem] || ''}`).join('');
...
function renderQimenCanonicalText(result, options = {}) {
    const detailLevel = (0, render_utils_js_1.normalizeDetailLevelBinary)(options.detailLevel);
